...
class Speaker():
    ...
    def infer_batch_bunch(self, sampling=False, train=False, featdropmask=None):
        """
...
class Speaker_v2():
    ...
    def __init__(self, tok, feature_size):
        self.tok = tok
        self.tok.finalize()
        ...
        print("VOCAB_SIZE", self.tok.vocab_size())
        # self.encoder = model.SpeakerEncoder(self.feature_size+args.angle_feat_size, args.rnn_dim, args.dropout, bidirectional=args.bidir).cuda()
        self.decoder = model.SpeakerDecoder_v2(self.tok.vocab_size(), args.wemb, self.tok.word_to_index['<PAD>'],
                                            args.rnn_dim, 
    ...
    def teacher_forcing(self, features, insts, lengths, for_listener=False, train=True):
        '''
            features: batchsize x length x feature_size
            insts: batchsize x ins length x feature_size
        '''
        if train:
            # self.encoder.train()
            self.decoder.train()
    ...
    def infer_batch(self, ctx, lengths, sampling=False, train=False, featdropmask=None):
        """
