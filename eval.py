...
class Evaluation(object):
    ...
    def _score_item(self, instr_id, path):
        ...
        self.scores['trajecotry_lengths_pair'][instr_id] = distance
        self.scores['steps_pair'][instr_id] = len(path) - 1
    ...
    def score(self, output_file):
        ...
        self.scores['trajecotry_lengths_pair'] = {}
        self.scores['steps_pair'] = {}
    ...
    def score_length(self, output_file):
        ''' Evaluate each agent trajectory based on how close it got to the goal location '''
        self.scores = defaultdict(list)
        self.scores['trajecotry_lengths_pair'] = {}
        self.scores['steps_pair'] = {}
        ...
        len_succ = [self.scores['trajectory_lengths'][i] for i,_ in enumerate(self.scores['nav_errors']) if _ < self.error_margin]
        len_fail = [self.scores['trajectory_lengths'][i] for i,_ in enumerate(self.scores['nav_errors']) if _ > self.error_margin]
        self.scores['lengths_succ'] = len_succ
        self.scores['lengths_fail'] = len_fail
        ...
        score_summary = {
            ...
            'steps': np.average(self.scores['trajectory_steps']),
            'lengths': np.average(self.scores['trajectory_lengths']),
            'lengths_succ': np.average(len_succ),
            'lengths_fail': np.average(len_fail)
        }
